...
class SharedFilesAdminSyncFiles {
  ...
  public function register_page_callback() {
    ...
    <div class="Xshared-files-admin-page-content-container shared-files-sync-files wrap">
      <h1><?php echo esc_html__('Sync Files', 'shared-files'); ?></h1>
      ...
      <p>
        <?php echo esc_html__('You may transfer many files at once using FTP to the folder mentioned below, and then sync the files here so they will be usable by the plugin.', 'shared-files') ?>
      </p>

      <p>
        <?php echo esc_html__('If a file is inactive, it means that it exists on the server, but has not yet been activated for the plugin.', 'shared-files') ?>
      </p>
      ...
      <p>
        ...
        <form method="post">
          ...
          <input type="submit" class="shared-files-activate shared-files-pro-required" value="<?php echo esc_html__('Activate all inactive files', 'shared-files') ?>" />

        </form>
      </p>

      <p>
        <?php echo esc_html__('Files found on the server at', 'shared-files') ?>
        <span class="shared-files-path"><?php echo esc_html( $path ) ?></span>:
      </p>
