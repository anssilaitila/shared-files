...
class SharedFilesFileUpload {
  ...
  public static function fileUploadMarkup($atts, $restrict_access = 0) {
    ...
    $html .= wp_nonce_field('sf_insert_file', 'secret_code', true, false);
    ...
    if ( $tag_count ) {
      ...
      } else if (isset($atts['tag_checkboxes']) || isset($s['show_tag_checkboxes_on_file_upload'])) {

        $taglist_args = ['taxonomy' => SHARED_FILES_TAG_SLUG, 'echo' => 0];
