...
class SharedFilesPublicFileCardDefault {
  ...
  public static function fileListItem($c, $imagefile, $hide_description, $show_tags = 0, $atts = [], $favorites = 0) {
    ...
    if (isset($s['card_align_elements_vertically'])) {
      $html = SharedFilesPublicFileCardVertical::fileListItem($c, $imagefile, $hide_description, $show_tags, $atts, $favorites);
      return $html;
    }
    ...
    if (
        isset( $s['card_featured_image_align'] ) &&
        $s['card_featured_image_align'] == 'left' &&
        isset( $s['card_featured_image_as_extra'] ) &&
        ( !$password || isset( $s['show_featured_image_for_password_protected_files'] ) ) &&
        ( !$file_access_logged_in_only || isset( $s['file_access_logged_in_only_show_featured_image'] ) ) &&
        !SharedFilesPublicHelpers::limitActive( $file_id ) &&
    ...
    if ( $show_file_link ) {
      ...
      $html .= '<a class="shared-files-file-title" ' . $nofollow .
        $data_file_type .
        $data_file_url .
        $data_video_url_redir .
        $data_external_url .
        $data_image_url .
        'href="' . esc_url( $file_url ) . '" target="_blank">' . sanitize_text_field( get_the_title() ) . '</a>';
    ...
    if (
      ( !isset($s['card_featured_image_align']) || $s['card_featured_image_align'] == '' ) &&
      isset( $s['card_featured_image_as_extra'] ) &&
      ( !$password || isset( $s['show_featured_image_for_password_protected_files'] ) ) &&
      ( !$file_access_logged_in_only || isset( $s['file_access_logged_in_only_show_featured_image'] ) ) &&
      !SharedFilesPublicHelpers::limitActive( $file_id ) &&
