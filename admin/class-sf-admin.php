...
class Shared_Files_Admin {
  ...
  public function enqueue_styles($hook) {
    ...
    if ($current_screen_id === 'edit-shared_file' || $current_screen_id === 'edit-shared-file-category' || $current_screen_id === 'shared_file_page_shared-files-shortcodes' || $current_screen_id === 'shared_file_page_shared-files-support') {
      ...
      wp_enqueue_style($this->plugin_name . '-tipso', SHARED_FILES_URI . 'dist/tipso.min.css', array(), $this->version, 'all');
      ...
      wp_enqueue_style( $this->plugin_name . '-google-fonts', 'https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&display=swap', false, 1 );
  ...
  public function enqueue_scripts($hook) {
    ...
    if ($current_screen_id === 'edit-shared_file' || $current_screen_id === 'edit-shared-file-category' || $current_screen_id === 'shared_file_page_shared-files-shortcodes' || $current_screen_id === 'shared_file_page_shared-files-support') {
      ...
      wp_enqueue_script($this->plugin_name . '-tipso', SHARED_FILES_URI . 'dist/tipso.min.js', array('jquery'), $this->version, true);
      wp_enqueue_script($this->plugin_name . '-clipboard', '/wp-includes/js/clipboard.js', array('jquery'), $this->version, true);
