    ...
    if ( current_user_can('manage_options') ) {
          ...
          echo "
            <div class='sf_notice sf_review_notice'>
              <div class='shared-files-notice-text'>
                ...
                '<div style=\'margin-bottom: 8px;\'><p style=\'font-size: 15px;\'>' .

                /* translators: %s: number of files added with the plugin */
                sprintf( __( "Hey, I noticed that you have added %s%d files%s with the Shared Files plugin – that's awesome!", 'shared-files' ), '<strong style=\'font-weight: 700;\'>', esc_attr( $file_cnt ), '</strong>' ) .

                '</p></div>' .

                '<div style=\'margin-bottom: 8px;\'><p style=\'font-size: 15px;\'>' . sprintf( __( "Could you please do me a BIG favour and give it a 5-star rating on WordPress? It will help to spread the word and boost our motivation.", 'shared-files' ) ) . '</p></div>' .

                '<p style=\'font-size: 15px;\'>– Anssi (' . esc_html__('plugin author', 'shared-files') . ')</p>' .
      ...
      if ( isset($screen->id) && $screen->id == 'edit-shared_file' ) {
        ...
        if ( $how_to_show_notice ) {
          ...
          echo wp_nonce_field('shared_files_ignore_how_to_notify', '_shared_files_ignore_how_to_notify_' . intval( $user_id ), true, false);
