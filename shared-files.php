...
if ( function_exists( 'shared_files_fs' ) ) {
  ...
  shared_files_fs()->set_basename( true, __FILE__ );
  ...
  function shared_files_fs_custom_connect_message(
    $message,
    $user_first_name
  ) {
    return sprintf(
      ...
      __( 'Hey %1$s' ) . ',<br>' .
      __( 'never miss an important update -- opt-in to our security and feature updates notifications, and non-sensitive diagnostic tracking with freemius.com.' ),
      $user_first_name
    );
  }

  shared_files_fs()->add_filter('connect_message', 'shared_files_fs_custom_connect_message', 10, 6);
  ...
  shared_files_fs()->add_filter( 'is_submenu_visible', 'shared_files_freemius_custom_is_submenu_visible', 10, 2 );
  ...
  function shared_files_fs_custom_connect_message_on_update(
    ...
    return sprintf(
      ...
      /* translators: %2$s: plugin's name */
      /* translators: %5$s: freemius link */
      __( 'Please help us improve %2$s! If you opt-in, some data about your usage of %2$s will be sent to %5$s. If you skip this, that\'s okay! %2$s will still work just fine.' ),
      $user_first_name,
      '<b>' . $plugin_title . '</b>',
      '<b>' . $user_login . '</b>',
      $site_link,
  ...
  shared_files_fs()->add_filter('connect_message_on_update', 'shared_files_fs_custom_connect_message_on_update', 10, 6);
   ...
   */
  define('SHARED_FILES_VERSION', '1.7.46');
