...
class SharedFilesPublicHelpers {
  ...
  public static function getFileURL($file_id = 0, $download = 0, $force_direct_url = 0, $atts = []) {
  ...
  public static function singleFileSimpleTitlesMarkup($atts = []) {
    ...
    $html .=  '<div class="shared-files-simple-list-col shared-files-simple-list-col-name shared-files-simple-list-col-title"><span>' .
              SharedFilesHelpers::getText('simple_list_title_file', __('File', 'shared-files')) .
              '</span></div>';
...
//    $file_url = (isset($c['_sf_filename']) ? SharedFilesHelpers::sf_root() . '/shared-files/' . intval( get_the_id() ) . '/' . SharedFilesHelpers::wp_engine() . sanitize_text_field( $c['_sf_filename'][0] ) : '');
    ...
    $html .= '<a class="shared-files-file-title" ' .
      $data_file_type .
      $data_file_url .
      $data_video_url_redir .
      $data_external_url .
      $data_image_url .
      'href="' . $file_url . '" target="_blank">' . sanitize_text_field( get_the_title() ) . '</a>';
